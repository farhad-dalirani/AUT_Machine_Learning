...
def runCodeB():
    ...
    for dim in [3, 5, 7]:
        ...
        allCostFunction[dim] = (costTrain,costTest)

    print allCostFunction
    color = {3:'r', 5:'b', 7:'g'}
    for dim in [3, 5, 7]:
        plt.plot(dim,allCostFunction[dim][0], color[dim]+'o', label="Train Dim "+str(dim))
        plt.plot(dim, allCostFunction[dim][1], color[dim] +'*', label="Test Dim "+str(dim))
        plt.title('Lambda: {}'.format(0))
        plt.ylabel('MSE')
        plt.xlim(1,9)
